...
//  MLX Code
//
//  Ported from AIStudio by Jordan Koch on 2026-08-18.
...
enum ModelRegistry {
    ...
    /// Map already-discovered local MLX model names into the registry. MLX Code
    /// discovers SafeTensors models from its own configured models directory
    /// (`MLXService.discoverModels()`) rather than the HF hub cache, so this thin
    /// mapper is the app-specific MLX discovery source. Pure and network-free.
    static func mlxModels(fromLocalNames names: [String]) -> [DiscoveredModel] {
        names.compactMap { name -> DiscoveredModel? in
            let trimmed = name.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !trimmed.isEmpty else { return nil }
            let short = trimmed.split(separator: "/").last.map(String.init) ?? trimmed
            return DiscoveredModel(modelName: trimmed, displayName: short, backend: .mlx, endpoint: "")
        }
    }
